...
export class HomePage
{
  ...
  ionViewDidLoad()
  {
    this.entryService.getEntry().then((res) => {
      this.items = res;
    },                                (err) => {
      ...
      (item) => {
        if (item)
        {
          this.entryService.createEntry(item).then((res) => {
            ...
            this.showAlert('Add Failed', 'An error occured when adding diary,please try again',
                           ['OK']);
  ...
  deleteItem(item)
  {
    this.entryService.deleteEntry(item).then(
      (res) => {
        ...
        this.showAlert('Delete Failed', 'An error occured when deleting diary, please try again',
                       ['OK']);
